...
class MarkdownGeneratorGUI:
    def __init__(self, root):
        ...
        # Directory Selection
        ttk.Label(self.main_frame, text="源目录:").grid(
            row=0, column=0, sticky=tk.W, pady=5
        )
        self.source_dir = tk.StringVar()
        ttk.Entry(self.main_frame, textvariable=self.source_dir, width=50).grid(
        ...
        ttk.Label(self.main_frame, text="输出文件:").grid(
            row=1, column=0, sticky=tk.W, pady=5
        )
        self.output_file = tk.StringVar(value="output.md")
        ttk.Entry(self.main_frame, textvariable=self.output_file, width=50).grid(
        ...
        # Include Extensions
        ttk.Label(filter_frame, text="包含的扩展名:").grid(
            row=0, column=0, sticky=tk.W, pady=5
        )
        self.include_extensions = tk.StringVar(
            value=".py,.txt,.md,.json,.yaml,.yml,.csv"
        )
        ttk.Entry(filter_frame, textvariable=self.include_extensions, width=50).grid(
            row=0, column=1, sticky=(tk.W, tk.E), pady=5
        )

        # Exclude Extensions
        ttk.Label(filter_frame, text="排除的扩展名:").grid(
            row=1, column=0, sticky=tk.W, pady=5
        )
        self.exclude_extensions = tk.StringVar(value=".pyc,.git,.pdf,.jpg,.png")
        ttk.Entry(filter_frame, textvariable=self.exclude_extensions, width=50).grid(
            row=1, column=1, sticky=(tk.W, tk.E), pady=5
        )

        # Exclude Directories
        ttk.Label(filter_frame, text="排除的目录:").grid(
            row=2, column=0, sticky=tk.W, pady=5
        )
        self.exclude_directories = tk.StringVar(
            value=".git,__pycache__,node_modules,.idea"
        )
        ttk.Entry(filter_frame, textvariable=self.exclude_directories, width=50).grid(
        ...
        ttk.Label(size_frame, text="最小文件大小 (bytes):").grid(
            row=0, column=0, sticky=tk.W
        )
        self.min_size = tk.StringVar(value="0")
        ttk.Entry(size_frame, textvariable=self.min_size, width=15).grid(
            row=0, column=1, padx=5
        )

        ttk.Label(size_frame, text="最大文件大小 (bytes):").grid(
            row=0, column=2, sticky=tk.W, padx=(20, 0)
        )
        self.max_size = tk.StringVar(value="1048576")  # 1MB
        ttk.Entry(size_frame, textvariable=self.max_size, width=15).grid(
        ...
        ttk.Label(format_frame, text="最大内容长度:").grid(
            row=0, column=0, sticky=tk.W, pady=5
        )
        self.max_content_length = tk.StringVar(value="100000000")
        ttk.Entry(format_frame, textvariable=self.max_content_length, width=20).grid(
            row=0, column=1, sticky=tk.W, pady=5
        )

        ttk.Label(format_frame, text="最大预览行数:").grid(
            row=1, column=0, sticky=tk.W, pady=5
        )
        self.max_preview_lines = tk.StringVar(value="10000")
        ttk.Entry(format_frame, textvariable=self.max_preview_lines, width=20).grid(
            row=1, column=1, sticky=tk.W, pady=5
        )

        self.show_binary_preview = tk.BooleanVar(value=False)
        ttk.Checkbutton(
            format_frame, text="显示二进制文件预览", variable=self.show_binary_preview
        ).grid(row=2, column=0, columnspan=2, sticky=tk.W, pady=5)

        # Generate Button
        ttk.Button(
            self.main_frame, text="生成Markdown", command=self.generate_markdown
